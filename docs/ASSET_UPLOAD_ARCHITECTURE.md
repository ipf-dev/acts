...
**실제 바이너리 파일은 S3 호환 Object Storage에 저장**하되, 외부 링크 에셋은
...
- frontend: 에셋 라이브러리 + 파일/링크 분리 업로드 모달 + 구조화 태그 편집 + 관리자 태그 관리

즉, 파일 에셋은 direct upload를 사용하고, 링크 에셋은 storage 없이 metadata-only로 등록한다.
...
이 구조를 쓰면 업로드와 AI 생성이 같은 에셋 모델을 공유한다.
...
5. 등록된 링크 에셋은 라이브러리 검색, 상세, 이력, 삭제, 내보내기 대상에 포함된다.
6. 프론트 에셋 라이브러리가 새 카탈로그를 다시 조회한다.
