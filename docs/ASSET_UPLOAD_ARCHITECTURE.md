...
- backend upload: `multipart/form-data -> Spring Boot -> LocalStack S3`
- metadata tables: `assets`, `asset_files`, `asset_tags`, `asset_events`
- frontend: 자산 라이브러리 + 업로드 모달 + 태그 편집

즉, 최종 목표는 direct upload까지 가더라도 **현재 동작 경로는 백엔드 경유 업로드**다.
이 단계에서는 구현 복잡도를 낮추고 메타데이터/태그/검색 경계를 먼저 고정하는 것이 목적이다.
...
1. 프론트 업로드 모달에서 여러 파일을 선택한다.
2. 각 파일에 대해 제목과 태그를 조정한다.
3. 프론트가 파일별로 `POST /api/assets/uploads` multipart 요청을 보낸다.
4. 백엔드가 S3(LocalStack)에 파일을 저장한다.
5. 백엔드가 `assets`, `asset_files`, `asset_tags`, `asset_events`를 저장한다.
