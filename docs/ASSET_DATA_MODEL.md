...
이 문서는 ACTS의 에셋 업로드, 자동 분류, 태깅, 중복 감지, 버전 관리,
외부 업로드, 제작 이력 관리 요구사항을 만족시키기 위한 **엔티티/테이블 기준안**이다.

핵심 목표:

- `한복 토니` 같은 검색어가 태그와 메타데이터를 통해 바로 검색될 것
- 멀티 파일 업로드와 외부 에셋 등록을 같은 에셋 모델로 다룰 것
- 에셋의 버전, 이력, 복구, 중복 판정을 추적할 것
...
ACTS에서는 **논리 에셋(asset)** 과 **실제 파일(file object)**, 그리고 **버전(version)** 을 분리한다.

- `asset`: 사용자가 검색하고 관리하는 대표 에셋
- `asset_version`: 에셋의 특정 시점 스냅샷
...
모든 에셋의 대표 레코드. 이미지/글/오디오/비디오/문서/기타를 여기서 시작한다.
...
- 에셋 상태 값 없이도 검색과 권한, 이력을 묶는 대표 anchor

설계 포인트:

- `search_text`는 제목, 설명, 파일명, 태그, 생성 루트, 캐릭터, 배경 등을 합쳐 저장하는 denormalized 컬럼이다.
- 텍스트 기반 시나리오 파일은 별도 `SCENARIO` 타입을 두지 않고 `DOCUMENT` 에셋 + `metadata.documentKind = SCENARIO`로 다룬다.
...
에셋의 특정 시점 버전. 복구 가능성을 위해 반드시 필요하다.
...
- 외부 에셋, AI 생성 결과, 내부 제작물을 같은 모델에 넣기 쉽다
