...
class AssetRepositoryImpl(
    ...
    override fun findCatalogFilterOptions(): AssetCatalogFilterOptionsResult {
        val organizations = entityManager.createQuery(
            """
            select distinct new com.acts.asset.repository.AssetCatalogOrganizationOptionResult(organization.id, organization.name)
            from AssetEntity asset
        ...
        val creators = entityManager.createQuery(
            """
            select distinct new com.acts.asset.repository.AssetCatalogCreatorOptionResult(asset.ownerEmail, asset.ownerName)
            from AssetEntity asset
            ${buildWhereClause(AssetListQuery())}
